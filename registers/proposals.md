# Proposals — unratified

**Moved here from `decided.md` on 2026-08-16, on Neil's instruction.** *"I want everything removed from
decided since that is being monitored by SRND OS… move it to proposals or something like that."*

## What this file is, and is not

**These are proposals, not decisions.** They were written by working sessions summarising conversations —
not ratified entry by entry — and they accumulated into something that read as settled fact and was cited as
authority. It was, repeatedly, including by the session that moved them here.

- **SRND OS is the source of record for what is actually decided.** Not this repo, and not this file.
- **Nothing here binds anything.** Cite an entry as *"a previous session recorded this"* — never as settled.
...
word, which makes this a register pass rather than a reading exercise — the method `registers/questions.md` already
runs. **What survives ratification belongs in SRND OS**, not back in this repo.

*Two things were dropped rather than moved: the "ratchet" framing in the old header, and a topic index added
earlier the same day — both asserted the contents were settled, which is the thing being corrected.*

*Evidence and dates are kept verbatim below, because the reasoning is the useful part and the attribution is
what makes ratification possible.*
...
| S16 | **A product data schema comes before any documentation work.** Capturing what *needs* to be documented precedes what is and will be documented; without it, the gap is an opinion rather than a measurable one. Field list in `../registers/record-schema.md`. **Completeness becomes a gate on new products.** | Neil's proposal. The Screen Wall — demonstrable since ISE 2023 with no page and no datasheet — is what its absence looks like. | 2026-07 |
...
| S2 | **Content does the rep's job** — reach, recall, the technical answer, teaching, and the whole specifier route. | `motion/sales-motion.md`. | 2026-07 |
...
| S8 | **Authorship is never scaled.** No bulk or generic content; volume is not the goal. The archive is legitimate because it is our own authored answers. | `motion/sales-motion.md`. | 2026-07 |
...
| # | Decision | Evidence / basis | Date |
|---|---|---|---|
| B1 | **Acoustic treatment design belongs to C-ATS.** Others cross-sell it; none duplicates it. | `group-strategy/the-group-play.md`. | 2026-07 |
...
| B8 | **Tools are never built around our own products**, and the report is the deliverable rather than tool access. | `motion/tools.md`, ADR 017/019/020/036. | 2026-07 |
