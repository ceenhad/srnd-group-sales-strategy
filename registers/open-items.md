...
**The repo was supposed to stop growing.** Since then it has gained `decided.md`, `evidence/current-state.md`,
`registers/record-schema.md`, a group document, and three brand files. Each had a stated reason, and the ratchet and
...
- **Reselling is easy and leaves you with nothing you control.** "Do too good and get fired, do bad also get
  ...
  `group-strategy/the-group-play.md` makes about dealer relationships one rung down, now with the group on the receiving
...
1. **Update the record, don't re-argue it.** Written into `group-strategy/commercial-model.md` (why direct, and why
   the test is deliberately hard) and `group-strategy/competitors.md` (the fork, answered).
...
3. **It settles what the partner programme is built on.** The distributor services — training, a demo room, a
   ...
   distinctiveness has to come from (`group-strategy/partner-programme.md`). Correction worth recording: this was
...
1. Pro-Fi described as "SRND Group's dedicated **spatial audio** brand" — a phrase Pro-Fi's own voice code
   ...
   — measured 2026-08-14** (`evidence/archive-findings.md` finding 28): three Mailchimp campaigns in January and February 2026
...
**The estate is also a decision waiting to be made rather than a channel** (`motion/content.md`): three live
...
Glasgow, the Yorkshire factory, sales leadership in Marbella. Written up in `group-strategy/commercial-model.md`.
...
1. **Content's job has two halves and we had only written the first.** Reaching dealers we cannot visit *and*
   ...
   now the standard every piece is judged against (`motion/content.md`).
...
3. **Institutionalising the moat and publishing the depth are one project**, not two (`group-strategy/the-group-play.md`).
4. **It bounds the partner programme's named technical contact**, which at our size is owner time and would
   otherwise rebuild the trap (`group-strategy/partner-programme.md`).
...
**Gap now addressed, with one decision left in it.** `motion/content.md` and `NEXT.md` lane 6 answer the
...
2. **The high-maintenance producers** — AV Nirvana, Prime Theaters, AV Daksma. They work, and they cost owner
   ...
   the direct base at the same time.
3. **AV Daksma also sells Screen Research.** Not necessarily a problem, but worth being a known and accepted
   position rather than an unexamined one.
4. **Retirement mechanics.** C1c says dead appointments go; what is not settled is the order, the notice, and
   whether a territory is re-approached directly straight away or left quiet for a period.
...
whose core market is shrinking, which sharpens stress-test items 2 and 3 rather than answering them. **The US
opening is the natural place to correct it**, because presenting the whole group there is the plan rather than a
retrofit.
...
`motion/sales-motion.md`: the list is **ranked, not exhaustive**, because owner time is finite; and the
...
**And the idea has now been worked to its limit** — it has its own document, `motion/sales-motion.md`. The
...
- **Stage 1 — buyer truth. IN PROGRESS; the foundation is confirmed so far.** From Neil's account of recent
  ...
  - **But don't mistake what happens now for what should happen — the correction that matters.** On any
    ...
    has to buy what's specified or argue their way out. Written up in `group-strategy/buyer-journey.md`, with the
    specifier channel gap in `group-strategy/channels.md`.
  ...
  - **Still to establish:** whether spec help is free or chargeable and whether that is deliberate; whether
    ...
    `motion/tools.md` wants tools to surface); and whether any recent work was non-cinema.
...
- **The `detect` / `record` / `route` primitives (`TSK-6`) stop being abstract.** `evidence/engine-audit.md` §2 already found
...
### The thesis behind it, in Neil's words — 2026-08-14
...
2. **The reconciliation map — every boundary, and what leaks at it.** Nine are already documented in
   `evidence/archive-findings.md`; the job is to complete the list, price each leak, and mark which disappear under
...
4. **And one sequencing question that decides the rest:** does the record (step 3) get filled *into* engine's
   knowledge mechanism now that we know it exists (`evidence/engine-audit.md` §2), rather than into files that later migrate?
...
Nine instances, counted in `evidence/archive-findings.md` finding 30 — C-ATS at six clicks across twenty-nine campaigns with
...
3. **Then the brand work runs as a swept question series, not as bespoke sessions.** *One list of questions,
   asked of every brand in turn* — the same instrument as `registers/questions.md`, applied per brand rather than per
...
> **That is the owner-driven road model the strategy exists to escape, at a smaller scale.** `commercial-model.md`
...
> page alone: **sixteen further `[decide]` calls live in `motion/standards.md`** — the dealer
...
- **▲ NEW — where free pre-sales ends and paid consulting begins.** Raised by the standards run (`T-S04`
  ...
  `motion/standards.md` session 2. **A priced ladder already exists on the tools side** —
  free calculators → the partner-gated tool → **£500 Pro Design, invoiced per engagement** → the full design
  service (`motion/tools.md`). What has no line at all is **human spec help**, which is where the cost
...
- ~~**▲ Is Apex-Tech actually gone?**~~ — **asked and answered, 2026-08-13. `C1f` and `C1j` stand unchanged.** The
  ...
  *"while we are starting our own push"* (Neil). The trading that looked live in DT's record was notice and grace,
  ...
  (`evidence/archive-findings.md` findings 9–10).
- **What "no product exists in two places" actually means — evidence from engine, 2026-08-13.** The record form's
  ...
  `srnd_store`** (DT-DYN, DT-HB, DT-MMD, DT-VMM, DT-PHG and eight more — `evidence/engine-audit.md` §1, group 11). Either the
...
- **Group identity line / positioning statement** — unresolved, but now anchored. Both current lines
  ...
  manufacturing for high-end environments"). It should **derive from the moat** (`group-strategy/the-group-play.md`):
...
- **Show dealers we value them — framework now designed, not yet built.** The relationship has been
  entirely one-way: dealers pay, and nothing goes back. `group-strategy/partner-programme.md` now sets the
  ...
  it's real. See `group-strategy/the-group-play.md`.
- **Leyard — entirely uncovered, and possibly an on-ramp asset.** The group's only genuine third-party
  distributed line (`group-strategy/commercial-model.md`) has had no strategy work at all. It matters more than
  its "carried line" status suggests: per `group-strategy/buyer-journey.md`, entry is governed by how much trust a
...
- **Entry products, per brand** — the low-risk, easy-to-specify item a stranger buys to try us, distinct
  from each brand's flagship (`group-strategy/buyer-journey.md`). Some are obvious (DT's smaller enclosures and port
  ...
  *mechanism* runs in engine already, and the programme behind it is now set out in
  ...
  - **▶ Two of these closed 2026-08-15 (Neil), and together they answer "what is registration actually
    ...
    *(Training is **provisional**: Neil is introducing something that bears on it.)* **This matters more than
    ...
    (`evidence/current-state.md` — *"registration exists; a partner programme does not"*), and these are the first two
...
- **Channel selection, priority & spend** — from the candidate landscape (`group-strategy/channels.md`),
  ...
  Evaluate each channel online-first, EI-style (`group-strategy/channels.md`).
- **Use the AVForums content credit (~£10k, owed, unused).** SRND built AVForums' editorial cinema
  ...
  - **So the live asset is the credit, spent on new content**, aimed at the Cinema Store range and the
    current products. See `group-strategy/commercial-model.md`, `group-strategy/channels.md`.
...
- **Cross-selling — the group's weakest link, and the thesis depends on it.** The whole model rests on a
  dealer won on one layer being served on the others (`group-strategy/the-group-play.md`), and we are currently poor at it. The nurture that would make it happen —
  permissioned, adjacency-aware partner-development campaigns timed to what a dealer is actually doing
  (`group-strategy/buyer-journey.md`) — does not exist. Two legs, in order:
  ...
  closing the gap (`motion/tools.md`). Build the adjacency map, close the practical gap,
  then market the ease — build it, then say it.
- **Cross-brand contact-cadence coordination** — the oversaturation risk
  (`group-strategy/commercial-model.md`); a group-level discipline with no brand home.
...
- Portal current-state observations (`group-strategy/commercial-model.md`) captured from the live
