...
> **This file is the model and the argument; the form is `registers/record-form.md`; the roster is
> `registers/product-register.md`.** Fill records against the form; change the model here and the form follows.
...
**The tasks are enumerated once, in [`motion/tasks.md`](../motion/tasks.md)** — 46 of them across a sales person's
...
- **Domain 11 is the missing link to the hook machinery.** `motion/motion-design.md` designs hooks by appeal
...
2. **RT5 already runs, and this file previously said it did not.** **SRND Engine's CRM does classic pipeline
   ...
   `X6` is a read from engine rather than a new habit. **Checking that is `registers/backlog.md` SYS-2**, and it is a Fetch.
3. **The two daily triggers already produce this and throw it away.** Every spec conversation fills the
   ...
   email and lost. The archive is the proof (`registers/backlog.md` CON-3) — **the knowledge exists, it has just never had
   anywhere to land.**
...
> per door, set by an owner, never generic (`registers/backlog.md` JNY-4). **The record says what a product can honestly be
