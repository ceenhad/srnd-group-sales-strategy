...
| SYS-2 | Do | **Capture the lost deal — one line: which competitor, which dimension, which stage.** The only outside view of the market we have, the whole content of the competitive group, and it is caught nowhere today. **Cheapest high-value habit in the register** | — | One line per loss | ready | `product-data-schema.md` §9 (`X6`) |
...
| TSK-2 | Decide | **The two unassigned execution tasks** — `T-M17` product launch and `T-M12` dealer marketing support. Ordinary marketing work nobody was given; the Screen Wall is what the first one costs and `N7` being empty for all 38 scopes is what the second one costs. Distinct from SYS-6's sensing cluster | owner | Minutes | ready | `group/10-tasks.md` |
| SYS-6 | Decide | **The four unowned outward-looking tasks are one job, not four gaps** — competitive response (`T-S11`), market intelligence (`T-S22`), product feedback (`T-S23`), competitor tracking (`T-M02`) — and with them, capturing why deals are lost. **Evidence against the existing "market intelligence is a recorded hole" position, not a reopening of it** (`decided.md` ratchet): the hole is wider than the one item. The decision is whether it becomes a role, a standing habit, or an accepted hole recorded at its true size | owner | Minutes | ready | `group/10-tasks.md` |
| SYS-7 | Do | **Assign the unowned marketing-executive tasks**, now that they are inventoried for the first time: competitor tracking, discoverability (search *and* AI answers), dealer marketing support, product launch, measurement instrumentation. Several need a habit rather than a person | — | A pass | ready | `product-data-schema.md` (the marketing week) |
| SYS-8 | Do | **Give product launch an owner and a checklist.** It is an unassigned *task*, not a content failure — the Screen Wall is what its absence costs. The checklist is the record's own gate: `D1`–`D10`, `G3`, `H2` and an asset audit before anything is announced | — | Small | ready | `product-data-schema.md` (the task map), `NEXT.md` lane 9 |
