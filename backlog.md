...
| TSK-2 | Decide | **The two unassigned execution tasks** — `T-M17` product launch and `T-M12` dealer marketing support. Ordinary marketing work with no owner — done ad hoc, landing nowhere; the Screen Wall is what the first one costs and `N7` being empty for all 38 scopes is what the second one costs. Distinct from SYS-6's sensing cluster | owner | Minutes | ready | `group/10-tasks.md` |
| SYS-6 | Decide | **The four unassigned outward-looking tasks are one job, not four gaps.** They happen informally; what is missing is an owner and somewhere the output lands — competitive response (`T-S11`), market intelligence (`T-S22`), product feedback (`T-S23`), competitor tracking (`T-M02`) — noting that **why deals are lost is already in engine's pipeline**; the question is whether it is structured for `X1`–`X6` (SYS-2). **Evidence against the existing "market intelligence is a recorded hole" position, not a reopening of it** (`decided.md` ratchet): the hole is wider than the one item. The decision is whether this becomes a role, a standing habit, or an accepted hole recorded at its true size | owner | Minutes | ready | `group/10-tasks.md` |
