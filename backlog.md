...
| MON-10 | Fetch | **Get margin or cost against the revenue lines.** **/ Part done 2026-08-13 - `archive-findings.md` finding 22.** Engine holds the **policy** (`brand_tier_margins` per brand per tier, versioned as price-list scenarios; `brand_tier_cost_multipliers` 1.6 distributor / 2.1 dealer / 3.0 RRP for the own-made brands) - but **of 165 live products only 6 carry a cost**, and DT's 85 and Fabric Walls' 37 carry none. **So policy yes, actuals no:** per-line realised margin is still not computable. Two things remain - **one sentence from Neil on whether `margin` is our margin or the channel's** (both readings fit; it changes the magnitude though not the direction), and **a cost column**, which `MON-11`'s re-export could carry The whole archive is revenue-only, and the one margin fact we have inverts a conclusion: **LED walls are 20 %** (Neil, 2026-08-13), so £610,885 of LED revenue is ~£122k of margin, while own-made lines at any normal manufacturing margin may out-earn them on half the revenue. **Every priority ranked by revenue is provisional until this exists** (`archive-findings.md` finding 18). Likely available from the same Xero reports with a cost column, or from engine's `products.cost` / `brand_tier_margins` | — | Small to obtain | **ready** | `archive-findings.md` finding 18, `engine-audit.md` §1 |
