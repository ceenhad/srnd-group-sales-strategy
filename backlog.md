...
| REC-2 | Do | Capture **where we win and lose against each named competitor** (T-S11) → defence is armed from history, not memory. **`REC-0` found this emptier than assumed, not richer:** engine records `won_at` and no loss reason, competitor or dimension at all, and has 11 weeks of history with 0 rejected quotes. **Not an engine read — a genuine capture build**, and the highest-value gap of the fifteen | — | Small | **ready** | qa-sheet pass 3, `engine-audit.md` §3 |
