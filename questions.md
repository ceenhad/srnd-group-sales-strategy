...
|---|---|---|---|---|
| **Q35** ⚑ | Services | **The product sweep Q34 called for.** Per product: what **project-specific design work** does it need in order to succeed? Then triage each — **automatable · tool-able · genuinely labour (which must be paid)**. Who runs it, and does it ride on the record (step 3) or run separately? | Rides on the record — it is a field per product, not a separate exercise | **Neil, 2026-08-16** |
