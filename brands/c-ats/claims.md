...
| **C1.4** | Most competitors sell acoustics on science and jargon | `brand` | confirmed 2026-08-18 |
...
| **C1.38** | ~~**Above direct-sell GIK, comparable to RPG, slightly below Artnovion**~~ **Some competitors sell direct to the end user at published prices; C-ATS is trade-only and gated, and carries the integrator's margin** | `brand` | **REPLACED 2026-08-19 — it never needed evidence, it needed the claim underneath it.** *`Q64` required a named comparison to be measured; the band was unmeasured, and `product-records.md` `N4` had already said so (*"`[?]` — price band only"*). But **the thing it was reaching for is a posture, not a price** — and posture is observable in one visit to a website. **The replacement is sayable, sourced (`C1.36`, `C1.37`, `commercial-model.md`) and names nobody.*** **This is axis `A5` doing its job** (`Q73`, `../../group-strategy/competitors.md` § acoustics): *ask what is **observable**, not what is measurable in principle — the same move as striking "measured performance"* |
