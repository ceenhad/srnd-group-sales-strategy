...
| **C1.41** | **The treatment layer belongs to neither of the two parties either side of it — which is why it is the layer that gets left late** | `brand` | **derived from `C1.40`, 2026-08-19, and stated as a derivation.** *The isolation layer has an owner (the architectural design) and the visible layer has an owner (the interior design). **The layer between them has no natural owner**, and a decision with no owner is the one that waits.* **This is the argument `T2` should make**, in place of the dimensions argument `C1.40` corrected |
