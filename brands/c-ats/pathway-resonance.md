...
| 6 | **G4** — first order | The RES-CP listing on `srnd.store` | Take the order faultlessly | Listing exists; the **£0.00 "Sale price" presentation is broken** (EST-4). **And the order cannot currently be sized** — see slot 2's gap | Order confirmation → the install material (slot 7) |
