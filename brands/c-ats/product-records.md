...
| 4 | Why do the absorption figures look low? | REF-CP | `known`, drafted | Because it is a scatterer. **Absent** — the coefficient-table article explains values above 1.0 and that mounting matters, never the low figures. **And `DOC-4` does not block the values: all three tables are written** **↻ THE REAL ANSWER, 2026-08-21 (`../../evidence/2026-08-21-cinema-tools-reports-read.md`): absorption is the wrong measure for this panel, and the figures are a design consequence rather than a limitation.** *The panel **removes nothing**. It splits one large reflection into many smaller, later ones until each falls under a level- and delay-dependent audibility threshold — Cinema Tools computes and reports that threshold per bounce. **A coefficient cannot express that.** The record has been carrying this as needing an explanation of scattering; the explanation is that the question mistakes the measure* |
