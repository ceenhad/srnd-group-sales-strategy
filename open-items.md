...
- **And the selling half still needs an owner.** Engine is thorough where it was authored — operations — and empty
  where it was not (`engine-audit.md` §0). **Making engine the hub for marketing and selling does not by itself
...
- **Show dealers we value them — framework now designed, not yet built.** The relationship has been
  entirely one-way: dealers pay, and nothing goes back. `group/03-partner-programme.md` now sets the
  ...
  it's real. See `group/00-strategy.md`.
- **Leyard — entirely uncovered, and possibly an on-ramp asset.** The group's only genuine third-party
  distributed line (`group/01-commercial-model.md`) has had no strategy work at all. It matters more than
  its "carried line" status suggests: per `group/02-buyer-journey.md`, entry is governed by how much trust a
...
- **Entry products, per brand** — the low-risk, easy-to-specify item a stranger buys to try us, distinct
  from each brand's flagship (`group/02-buyer-journey.md`). Some are obvious (DT's smaller enclosures and port
...
- **Partner programme — designed; the remaining decisions are narrow.** The registration and gating
  *mechanism* runs in engine already, and the programme behind it is now set out in
  `group/03-partner-programme.md` (what registration grants — including training access; the Partner
...
- **Channel selection, priority & spend** — from the candidate landscape (`group/05-channels.md`),
  ...
  Evaluate each channel online-first, EI-style (`group/05-channels.md`).
- **Use the AVForums content credit (~£10k, owed, unused).** SRND built AVForums' editorial cinema
  ...
  - **So the live asset is the credit, spent on new content**, aimed at the Cinema Store range and the
    current products. See `group/01-commercial-model.md`, `group/05-channels.md`.
...
- **Cross-selling — the group's weakest link, and the thesis depends on it.** The whole model rests on a
  dealer won on one layer being served on the others (`group/00-strategy.md`), and we are currently poor at it. The nurture that would make it happen —
  permissioned, adjacency-aware partner-development campaigns timed to what a dealer is actually doing
  (`group/02-buyer-journey.md`) — does not exist. Two legs, in order:
  ...
  closing the gap (`group/07-tools.md`). Build the adjacency map, close the practical gap,
  then market the ease — build it, then say it.
- **Cross-brand contact-cadence coordination** — the oversaturation risk
  (`group/01-commercial-model.md`); a group-level discipline with no brand home.
...
- Portal current-state observations (`group/01-commercial-model.md`) captured from the live
