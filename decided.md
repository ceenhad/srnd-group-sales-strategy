...
| C1h | **US entry leads with C-ATS, not DT.** Smallest first order, the only brand documented to a standard, the only content with demonstrated search pull, and it ships economically. DT is the revenue; C-ATS is the door. | Reasoned from the entry-product logic and the estate's own numbers. | 2026-07 |
| C1i | **The Screen Innovations white label must be resolved before DT enters the US** — continue it and DT stays out, or bound/end it and DT enters in its own name. Entering without choosing puts us in competition with our own white-label customer. | `01-commercial-model.md`. **Open decision, not yet made.** | 2026-07 |
