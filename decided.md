...
| S16a | **Engine is the system of record for product data; `../product-data-schema.md` is the requirements view.** Much of the schema work is likely already under consideration or partly implemented in Engine, so the first move is to check it and reconcile — where Engine models a field, this repo defers and records the mapping. Two competing schemas would be worse than none. | Neil's note. | 2026-07 |
