...
*Quoting the argument already made in `registers/open-items.md`, not re-deriving it:* **a KPI is only real if it has one
...
of owner answers feed the recurring-question list (`evidence/current-state.md`).
...
`evidence/current-state.md`): no exposure or collections alarm belongs in this set; at most a payments-on-schedule flag
...
filled — `registers/product-register.md` counts the gaps) · and the stall signal, which needs no instrumentation: a month
with no publication.
...
Built from the boundaries `evidence/archive-findings.md` and `registers/backlog.md` document. Figures below are only ones that
